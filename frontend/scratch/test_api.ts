...
async function runTests() {
  ...
  try {
    ...
    console.log(
      `User ID: ${registerResponse.user.id}, Role: ${registerResponse.user.role}`,
    );
    console.log(
      `Extracted Token: ${registerResponse.access_token.substring(0, 15)}...`,
    );
    ...
    apiClient.defaults.headers.common["Authorization"] =
      `Bearer ${registerResponse.access_token}`;
    ...
    try {
      ...
      if (e instanceof ApiError) {
        console.log(
          "✅ ApiError Interceptor seamlessly caught a 400 Bad Request!",
        );
    ...
    console.log(
      `✅ getAllRfqs Native Unwrap Successful! Found ${rfqs.length} total active RFQs.`,
    );
    if (rfqs.length > 0) {
