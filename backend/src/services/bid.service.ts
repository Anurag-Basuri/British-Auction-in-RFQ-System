 ...
 */
export async function createBid(rfqId: number, supplierId: number, dto: { freight_charges: number, origin_charges: number, destination_charges: number, transit_time: string, quote_validity: Date }) {
  const timestamp = new Date();
  
  const price = dto.freight_charges + dto.origin_charges + dto.destination_charges;
  ...
  return prisma.$transaction(async (tx: any) => {
    ...
    const extensionData = await evaluateExtension(tx, rfqId, supplierId, price, timestamp);
    ...
    if (extensionData) {
      await tx.extensionLog.create({
        data: {
          ...
          extended_mins: Math.round((extensionData.newClose.getTime() - previousClose.getTime()) / 60000),
