...
export const createRfqController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    res
      .status(201)
      .json(new ApiResponse(201, result, "RFQ created successfully"));
  },
);

export const findAllRfqController = asyncHandler(
  async (_req: Request, res: Response) => {
    const result = await rfqService.findAll();
    res
      .status(200)
      .json(new ApiResponse(200, result, "RFQs retrieved successfully"));
  },
);

export const findOneRfqController = asyncHandler(
  async (req: Request, res: Response) => {
